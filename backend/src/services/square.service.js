...
// createClient(creds) — creds is { accessToken, baseUrl, configured }, e.g.
// config.square (one shared account, used below for the default export) or
// config.restaurantSquare.forCompanyCode('SBR') (Restaurant module Phase 4:
// each restaurant has its own separate Square account). Every method below
// is a plain function bound to that one set of credentials, so two callers
// can hold two independent clients — e.g. importing Star Bar Restaurant and
// Bamboo Garden's Square history without either token leaking into the
// other's requests.
...
module.exports = Object.assign({ createClient: createClient }, createClient(config.square));
