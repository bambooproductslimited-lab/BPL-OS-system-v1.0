  ...
  return (
    <div className="mkt-reco">
      ...
      <p className="field-hint">
        {tr('Based on this account\'s own post and channel numbers from the last 90 days — what to post more of, and which channel or campaign needs a promotional push.')}
      </p>
      {error && <div className="error-banner">{error}</div>}
      {result && <p className="mkt-reco-text">{result.recommendation}</p>}
      {!result && !loading && !error && <p className="table-empty">{tr('Not generated yet for this session.')}</p>}
    </div>
  );
}
