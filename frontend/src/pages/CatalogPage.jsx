  ...
  return (
    <div>
      ...
      <div className="catalog-toolbar">
        <SearchInput value={search} onChange={setSearch} placeholder={tr('Search items, categories, variations…')} />
        {canManage && <button type="button" className="btn btn-primary" onClick={openNewItem}>{tr('Add item')}</button>}
      </div>

      <table className="table">
        <thead>
          <tr><th /><th>{tr('Item')}</th><th>{tr('Category')}</th><th>{tr('Variations')}</th><th>{tr('Stock')}</th><th>{tr('Status')}</th><th /></tr>
        </thead>
        <tbody>
                ...
                <tr>
                  ...
                  <td colSpan={6} className="catalog-variations-cell">
                    <table className="table catalog-variations-table">
                      <thead>
                        <tr><th>{tr('Variation')}</th><th>{tr('Code')}</th><th>{tr('Unit')}</th><th>{tr('Unit price')}</th><th>{tr('Cost price')}</th><th>{tr('Stock')}</th><th>{tr('Status')}</th><th /></tr>
                      </thead>
                      <tbody>
                    ...
                    {canManage && <button type="button" className="btn btn-secondary catalog-add-variation" onClick={() => openNewVariation(item.id)}>{tr('+ Add variation')}</button>}
                  </td>
                </tr>
              )}
            </Fragment>
      ...
      {!items.length && (
        <div className="catalog-empty-state">
          ...
          <p className="catalog-empty-title">{tr('No catalogue items yet')}</p>
        </div>
      )}
